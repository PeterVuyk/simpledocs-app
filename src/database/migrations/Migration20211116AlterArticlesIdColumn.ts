  ...
  implements Migration
{
  ...
  public up(sqlTransaction: SQLTransaction): void {
    ...
    sqlTransaction.executeSql(
      'insert into articles_dg_tmp(id, chapter, pageIndex, title, bookType, subTitle, content, contentType, searchText, chapterDivision, iconFile, bookmarked) select random(), chapter, pageIndex, title, bookType, subTitle, content, contentType, searchText, chapterDivision, iconFile, bookmarked from articles;',
    );
    sqlTransaction.executeSql('drop table articles;');
    sqlTransaction.executeSql(
