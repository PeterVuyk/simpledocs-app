  ...
  return new Promise((resolve, reject) => {
    db.transaction(
      sqlTransaction => {
        ...
        sqlTransaction.executeSql(
          'create table if not exists articles (id integer not null constraint articles_pk primary key autoincrement, chapter varchar not null, pageIndex integer not null, title text not null, bookType text not null, subTitle text, content blob not null, contentType varchar not null, searchText text not null, chapterDivision varchar not null, iconFile blob);',
        );
        sqlTransaction.executeSql(
