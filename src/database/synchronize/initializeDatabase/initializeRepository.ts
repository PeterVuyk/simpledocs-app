  ...
  return new Promise((resolve, reject) => {
    db.transaction(
      sqlTransaction => {
        ...
        // When merging migrations before release, rename 'articles_pk' to 'bookPages_pk'
        sqlTransaction.executeSql(
          'create table if not exists bookPages (id varchar not null constraint articles_pk primary key, chapter varchar not null, pageIndex integer not null, title text not null, bookType text not null, subTitle text, content blob not null, contentType varchar not null, searchText text not null, chapterDivision varchar not null, iconFile blob, bookmarked integer default 0 not null);',
        );
        sqlTransaction.executeSql(
